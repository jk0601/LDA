import nltk
try:
    # 필요한 NLTK 데이터셋만 다운로드 (필요한 것만 나열)
    nltk.download('punkt', quiet=True)
    nltk.download('stopwords', quiet=True)
    # 필요하다면 추가 데이터셋 다운로드
except Exception as e:
    print(f"NLTK 데이터 다운로드 오류: {e}")
...
class TextMiningAnalysis:
    def __init__(self, file_path=None, text_column=None, language='korean'):
        ...
        self.corpus = []
        if file_path:
            # 파일이 존재하는지 확인하고 전체 경로 직접 사용
            if os.path.exists(file_path):
                full_path = file_path
            else:
                # 파일이 data 디렉토리에 있을 수도 있으므로 확인
                basename = os.path.basename(file_path)
                full_path = os.path.join(self.data_dir, basename)
            ...
            if file_path.endswith('.csv'):
                try:
                    ...
                    try:
                        ...
                        # UTF-8 실패시 CP949(한국어 Windows 기본 인코딩) 시도
                        try:
                    ...
                    if text_column and text_column in df.columns:
                        ...
                        print(f"데이터 로드 완료: {len(self.corpus)}개 문서")
                        # 잘 로드되었는지 첫 5개 항목 출력
                        for i, text in enumerate(self.corpus[:5]):
                            print(f"문서 {i} 미리보기: {text[:50]}...")
                    else:
                        available_columns = ', '.join(df.columns)
                        print(f"텍스트 컬럼 '{text_column}'이 존재하지 않습니다. 가능한 컬럼: {available_columns}")
                except Exception as e:
    ...
    def _load_sentiment_dict(self):
        ...
        try:
            ...
            if not os.path.exists(sentiment_path):
                print("KNU 감성사전을 다운로드합니다...")
                knu_tm = KnuTextMining()
                return knu_tm.sentiment_dict
    ...
    def preprocess_text(self, min_word_length=1, pos_filter=None, custom_stopwords=None):
        ...
        print(f"전처리 시작: 총 {len(self.corpus)}개 문서")
        
        # 문서별 처리
        empty_docs = 0
        for doc_idx, doc in enumerate(self.corpus):
            if not isinstance(doc, str):
                print(f"경고: 문서 {doc_idx}가 문자열이 아닙니다: {type(doc)}")
                empty_docs += 1
                continue
            ...
            if len(tokens) == 0:
                empty_docs += 1
                if doc_idx < 5:  # 처음 몇 개 문서에 대해서만 상세 정보 출력
                    print(f"경고: 문서 {doc_idx}에서 토큰화 후 단어가 없습니다. 원본: '{doc[:50]}...'")
            
            # 결과 저장
            self.tokenized_corpus.append(tokens)
            self.processed_corpus.append(' '.join(tokens))
        
        print(f"전처리 완료: {len(self.corpus)}개 문서 중 {empty_docs}개 빈 문서 발견")
        print(f"토큰화된 문서 수: {len(self.tokenized_corpus)}")
        
        # 전체 토큰이 없는 경우 경고
        total_tokens = sum(len(tokens) for tokens in self.tokenized_corpus)
        if total_tokens == 0:
            print("경고: 모든 문서에서 토큰이 추출되지 않았습니다. 텍스트 컬럼을 확인하세요.")
        else:
            print(f"총 추출된 토큰 수: {total_tokens}")
    ...
    def perform_tf_idf_analysis(self, max_features=1000):
        ...
        if not self.processed_corpus:
            print("오류: 처리된 텍스트가 없습니다. preprocess_text()를 먼저 호출하세요.")
            return
            
        # 빈 문서 확인
        empty_docs = [i for i, doc in enumerate(self.processed_corpus) if not doc.strip()]
        if empty_docs:
            print(f"경고: {len(empty_docs)}개의 빈 문서가 있습니다. 이들은 분석에서 제외됩니다.")
            self.processed_corpus = [doc for doc in self.processed_corpus if doc.strip()]
            
        if not self.processed_corpus:
            print("오류: 모든 문서가 비어있습니다. 텍스트 컬럼이나 전처리 설정을 확인하세요.")
            return
        ...
        try:
            ...
            print(f"TF-IDF 분석 완료: {len(self.tf_idf_feature_names)}개 특성 추출")
        except ValueError as e:
            print(f"TF-IDF 분석 오류: {e}")
            print("가능한 원인: 문서가 비어있거나, 모든 단어가 불용어로 처리되었을 수 있습니다.")
            print("해결 방법: min_word_length를 줄이거나, 불용어 목록을 수정하거나, 품사 필터를 조정하세요.")
            self.tf_idf_matrix = None
            self.tf_idf_feature_names = None
    ...
    def plot_network(self, graph, figsize=(12, 12)):
        ...
        # 한글 폰트 설정 (plot_network 함수 내부에 직접 추가)
        ...
        nx.draw_networkx_labels(graph, pos, font_size=12, font_family='Malgun Gothic')

...
import logging
logging.basicConfig(level=logging.WARNING)  # INFO 대신 WARNING으로 변경
...
if not jpype.isJVMStarted():
    try:
        ...
        if os.path.exists(jvm_path):
            ...
            print(f"JVM 시작됨: {jvm_path}")
        else:
            print(f"JVM 라이브러리를 찾을 수 없습니다. 경로: {jvm_path}")
...
if platform.system() == 'Windows':
    ...
    FONT_PATH = '/usr/share/fonts/truetype/nanum/NanumGothic.ttf'  # Linux 경로
...
def analyze():
    try:
        ...
        try:
            wordcloud = analyzer.create_word_cloud(FONT_PATH)
            ...
            plt.title('전체 키워드 워드클라우드', fontsize=16, fontfamily='Malgun Gothic')
            plt.axis('off')
            plt.tight_layout()
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', debug=True, threaded=False, use_reloader=False)